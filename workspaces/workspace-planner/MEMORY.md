...
**自己创建的技能**（带 `0-` 前缀）→ 已经学会，直接记录到"已学习技能"

**同步的技能**（无前缀）→ 不需要学习，属于外部安装
...
**技能位置**：`~/.openclaw/workspace-planner/skills/0-nanobanana-ppt/`

**核心能力**：
- 基于 Google Gemini AI 生成高质量 PPT 图片
- 支持智能视频转场效果
- 交互式播放支持
- 可灵AI API 集成（可选）
...
**使用方法**：
```bash
cd ~/.openclaw/workspace-planner/skills/0-nanobanana-ppt
source venv/bin/activate
python -m nanobanana --topic "主题" --slides 10 --output ./output
```
...
**网站地址**：https://colors.lol/

**用途**：查找和参考配色方案，用于PPT、网页设计

**使用场景**：制作演示页面、PPT配色方案设计时参考
